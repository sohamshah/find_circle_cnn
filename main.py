...
def train_model():
    ...
    circle_locations = np.zeros((numimages, 1, 3), dtype=np.float) #num circles in image = 1, number of outputs req'd = 3 (r,c,rad)
    ...
    cnn.add(Conv2D(64, kernel_size=3, strides=(1,1), data_format='channels_last',input_shape=(200,200,1), padding='same', kernel_initializer='glorot_normal'))
...
def main():
    ...
    for _ in range(1000):
        params, img = noisy_circle(200, 50, 2) #noisy circles in 200*200 pixel matrix (ndarray), with radius from 10 to 50, and noise=2
